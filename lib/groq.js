  ...
  for (const model of modelsToTry) {
    try {
      ...
      if (!res.ok) {
        ...
        console.warn(`[Groq] Model '${model}' returned HTTP ${res.status}: ${errText.slice(0, 150)}. Retrying with fallback model...`);
      ...
      console.warn(`[Groq] Model '${model}' failed: ${err.message}. Retrying with fallback model...`);
      lastError = err;
      continue;
    }
  }
...
function lessonSystemPrompt(level, language) {
  const levels = {
    ...
    school: 'Explain at Class 10 (high-school) level. Extremely clear, comprehensive, and thoroughly explained with step-by-step reasoning, real-world examples, and precise scientific/academic terms.',
    exam: 'Explain at exam-ready depth. Extremely detailed, precise, comprehensive; include formulas, definitions, diagrams, and step-by-step derivations that an examiner expects for full marks.',
  };
...
If they wrote Hindi script, reply in clear Hindi script. Keep standard technical/scientific terms in
English where that's how students actually say them (e.g. "photosynthesis", "derivative").`;
  } else {
    langRule = `CRITICAL — NON-NEGOTIABLE LANGUAGE COMPLIANCE:
Write the ENTIRE lesson (title, subject, headings, steps, explanation text AND narration) strictly in: ${language}.
Do NOT switch to English mid-lesson (except for standard scientific/technical terms like "photosynthesis" or "equation").`;
  }

  return `You are a world-class AI master teacher who turns any student doubt into an exceptionally clear, deep, intuitive slide deck.
...
Return ONLY valid JSON (no markdown, no backticks) in EXACTLY this shape:
{
  ...
  "slides": [
    ...
    { "type": "explanation", "heading": "...", "points": [ {"bold":"Key idea","text":"a full 2-4 sentence detailed, clear explanation of this idea with real-world analogies"}, ... ], "hasPhoto": false }
  ],
  "narration": ["spoken explanation for slide 1", "spoken explanation for slide 2", "..."]
}
Rules for MAXIMUM CLARITY & EXPLANATION QUALITY:
- You MUST provide between 7 and 10 slides in the "slides" array. ABSOLUTELY NEVER return fewer than 7 slides.
- PHOTO MANDATE: Set "hasPhoto": true on EXACTLY 3 or 4 slides in the deck where a visual photo is most helpful. Set "hasPhoto": false for all other slides. Include a unique, specific "searchKeyword" ONLY on slides where "hasPhoto" is true.
- Always include 1 Title slide, at least 1 "flowchart" slide, at least 2 "diagram" slides, and 3 to 6 "explanation" slides.
- For "flowchart" slides, "steps" must contain 4 to 6 logical, chronological steps of a process. Ensure they are clear and self-explanatory.
- For "diagram" slides, "parts" must contain 3 to 6 key components with concrete labels ("label") and descriptive roles ("desc").
- For "explanation" slides, EVERY point MUST carry a full 2-4 sentence thorough explanation in "text", not just a short label.
- "narration" MUST have exactly ONE entry per slide. Each narration entry is the tutor's actual SPOKEN TEACHING of that slide: 3 to 5 full, rich sentences (roughly 50–90 words) that genuinely and thoroughly EXPLAIN the concept on that slide in plain, crystal-clear words, with step-by-step logic and everyday examples. It must teach the idea itself — NEVER a pointer phrase like "as you can see" or "let's look at the diagram".
- STRICT UNIFORM LANGUAGE & DEPTH: Provide the exact same depth, richness, and completeness no matter which language is selected. In ${language}, write every narration and explanation text fully and clearly in ${language} from start to finish.`;
}
...
export async function generateLesson({ doubt, level = 'school', language = 'English' }) {
  const data = await groqChatCompletion({
    model: process.env.GROQ_LESSON_MODEL || 'llama-3.3-70b-versatile',
    temperature: 0.4,
    max_tokens: 4096,
...
export async function readImageDoubt({ imageDataUrl }) {
  try {
    ...
    try {
      ...
      const recognize = Tesseract.recognize || Tesseract;
      const base64Data = String(imageDataUrl || '').replace(/^data:image\/\w+;base64,/, '');
      if (base64Data && base64Data.length > 100) {
        const imgBuffer = Buffer.from(base64Data, 'base64');
        const res = await recognize(imgBuffer, 'eng').catch(() => ({ data: { text: '' } }));
        rawOcrText = (res && res.data && res.data.text ? res.data.text : '').trim();
      }
    } catch (err) {
      console.warn('[Vision OCR] Tesseract note:', err.message);
    }

    if (rawOcrText && rawOcrText.length > 3) {
      try {
        const res = await groqChatCompletion({
          model: 'llama-3.1-8b-instant',
          temperature: 0.2,
          messages: [
            { role: 'system', content: 'You are an AI tutor analyzing OCR text from a textbook page or problem. State the exact academic question or concept in 1 clear, concise sentence.' },
            { role: 'user', content: `Text from photo:\n"${rawOcrText}"` },
          ],
        });
        const question = res.choices[0].message.content.trim();
        if (question) return question;
      } catch (_) {}
      return rawOcrText.slice(0, 150);
    }
  } catch (outerErr) {
    console.warn('[Vision OCR] Outer error:', outerErr.message);
  }

  return 'Explain the science and math concepts shown in this textbook problem.';
}
