...
export async function synthesize({ text, language = 'English', lessonTitle = '' }) {
  ...
  const cartesiaKey = (envKey && envKey.startsWith('sk_car_')) ? envKey : 'sk_car_1jFrVMrJN3WDuB1iwRF9Sr';
  const langKey = resolveLangKey(language);

  let voiceId;
  if (langKey === 'English') {
    voiceId = process.env.CARTESIA_VOICE_ID_ENGLISH || '7ac18ded-a790-43a2-934c-0986e18292dd';
  } else if (langKey === 'Hindi' || langKey === 'Hinglish') {
    voiceId = process.env.CARTESIA_VOICE_ID_HINDI || 'c3edccd2-23da-41f8-83f0-f34f8d985eb3';
  } else if (langKey === 'Kannada' || langKey === 'Kanglish') {
    voiceId = process.env.CARTESIA_VOICE_ID_KANNADA || 'e7f56b14-ab11-49d9-b168-f0d5ab2ae463';
  } else {
